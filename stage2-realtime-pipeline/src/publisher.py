...
# Load secrets from a local .env file (see .env.example) instead of hardcoding them.
# SECURITY NOTE: this file used to contain a real NSW FuelCheck API key/secret in
# plain text. They were removed before this project was organised for GitHub.
# Treat those old values as compromised — rotate/regenerate them on the FuelCheck
# developer portal before relying on this project again.
load_dotenv()

API_Key = os.environ["FUELCHECK_API_KEY"]
API_secret = os.environ["FUELCHECK_API_SECRET"]
Auth_Header = os.environ["FUELCHECK_AUTH_HEADER"]
# URL for NSW API
...
def run_service():
    ...
    df = clean_dataset(df)
    # Publish cleaned data to MQTT broker
...
if __name__ == "__main__":
    # entry point of the script: get API access token and run the service
